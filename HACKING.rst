...
To get the required version into your ``venv``, run ``tox`` once.  Alternatively, ``pip install`` the ``python-apt`` version at the URL listed in the ``tox.ini`` section ``[test]``.  After that, you can run ``pytest`` as normal.
