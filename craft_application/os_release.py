...
class OsRelease:
    ...
    def id(self) -> str:
        ...
        :raises CraftError: If no ID can be determined.
        """
        with contextlib.suppress(KeyError):
            return self._os_release["ID"]

        raise CraftError("Unable to determine host OS ID")

    def name(self) -> str:
        """Return the OS name.

        :raises CraftError: If no name can be determined.
        """
        with contextlib.suppress(KeyError):
            return self._os_release["NAME"]

        raise CraftError("Unable to determine host OS name")

    def version_id(self) -> str:
        """Return the OS version ID.

        :raises CraftError: If no version ID can be determined.
        """
        with contextlib.suppress(KeyError):
            return self._os_release["VERSION_ID"]

        raise CraftError("Unable to determine host OS version ID")
    ...
    def version_codename(self) -> str:
        ...
        :raises CraftError: If no version codename can be determined.
        """
        with contextlib.suppress(KeyError):
        ...
        raise CraftError("Unable to determine host OS version codename")
