...
from craft_application.util import is_running_from_snap

_overview = """
Run tests in different back-ends.

This command will run test suites using the spread tool. For further
...
class TestCommand(AppCommand):
    ...
    def run(self, parsed_args: argparse.Namespace) -> None:
        ...
        if is_running_from_snap(self._app.name):
        ...
        spread_environment = self._get_spread_environment(parsed_args)

        try:
            with emit.pause():
                subprocess.run(
                    args=[*cmd, *spread_tasks],
                    env=spread_environment,
                    check=True,
                )
