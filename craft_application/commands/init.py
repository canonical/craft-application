...
class InitCommand(base.AppCommand):
    ...
    @property
    def template_dirs(self) -> list[pathlib.Path]:
        """A list of directories for all templates."""
        return [path for path in self.parent_template_dir.iterdir() if path.is_dir()]

    @property
    def profiles(self) -> list[str]:
        """A list of profile names generated from template directories."""
        return [template.name for template in self.template_dirs]
