...
from craft_application import errors, os_release
...
def _os_release(new_dir):
    def _release_data(contents):
        ...
        return os_release.OsRelease(os_release_file=path)

    return _release_data


def test_blank_lines(_os_release):  # noqa: PT019
    release = _os_release(
        dedent(
            """\
            NAME="Arch Linux"
...
def test_no_id(_os_release):  # noqa: PT019
    release = _os_release(
        dedent(
            """\
            NAME="Arch Linux"
...
def test_no_name(_os_release):  # noqa: PT019
    release = _os_release(
        dedent(
            """\
            ID=arch
...
def test_no_version_id(_os_release):  # noqa: PT019
    release = _os_release(
        dedent(
            """\
            NAME="Arch Linux"
...
def test_no_version_codename(_os_release):  # noqa: PT019
...
def test_no_version_codename_or_version_id(_os_release):  # noqa: PT019
    release = _os_release(
        dedent(
            """\
            NAME="Ubuntu"
