...
def test_run_sets_platform_arg(
    ...
    assert build_planner._BuildPlanService__platforms == [fake_platform]  # type: ignore[reportAttributeAccessIssue]  # ty: ignore[unresolved-attribute]
...
def test_run_sets_build_for_arg(
    ...
    assert build_planner._BuildPlanService__build_for == [arch]  # type: ignore[reportAttributeAccessIssue]  # ty: ignore[unresolved-attribute]
...
def test_pack_run(
    ...
    command._services.package.resource_map = {p.stem: p for p in packages[1:]} or None  # pyright: ignore[reportAttributeAccessIssue]  # ty: ignore[invalid-assignment]
