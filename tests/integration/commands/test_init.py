...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(None, pathlib.Path("simple-file"), id="default"),
...
def test_init_invalid_profile(app, capsys, monkeypatch):
    ...
    expected_error = "Error: argument --profile: invalid choice: 'bad' (choose from 'simple', 'other-template')"
...
def test_init_overlapping_file(app, capsys, monkeypatch, tmp_path):
    ...
    expected_error = (
        f"Cannot initialise project in {str(tmp_path)!r} because it would overwrite existing files.\n"
        "Existing files are: \n  - simple-file\n"
